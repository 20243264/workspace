...
*p=100; // 포인터변수 p에 저장된 주소에 100을 대입(num=100 동일)
*2.포인터가 아무것도 가리키지 않은 경우 NULL(0) 설정 
*-NULL은 <stdio.h>에서 정수 0으로 정의
*-주소0을 엑세스하면 시스템에서 자동적으로 오류 감지 
...
int main() {
    int a; //일반 변수 a 선언 (10번지~13번지)
    int *pa; //포인터 변수 pa 선언 
    //pa->포인터 변수
    //&a->a변수의 시작주소
    pa=&a; //포인터 변수 pa에 10번지(주소) 저장
    //*pa(간접참조연산자):포인터 변수 pa의 실제 주소에 99 값을 저장 
    //포인터가 가리키는 위치의 값을 추출
    //()다음으로 높은 우선순위를 가짐->다른 연산자보타 먼저 실행됨
    *pa=99;
    //*pa==a 동일
    printf("포인터로 a 출력:%d\n",*pa);
    printf("변수로 a 출력:%d\n",a);
