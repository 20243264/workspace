...
*-포인터변수를 사용하는 방법은 변수 선언 및 주소연산, 간접참조 연산 등 복잡(이것만 보면 포인터변수를 사용할 이유가 없음)
*->개발시 포인터를 즐겨 사용할 필요 없음!
*임베디드 프로그래밍을 할 때 메모리에 직접 접근하는 경우 또는 동적할당한 메모리를 사용하는 경우에 포인터가 반드시 필요함(+swap)
...
int main() {
    ...
    //1.call-by-value(깂에 의한 호출)
    //-함수 호출시 매개변수로 값의 복사본이 전달 
    //-함수 내에서 매개변수를 변경해도 원본값에는 영향을 주지 않음
    //2.call-by-reference(함수에 의한 호출)
    //-함수 호출시 원본(주소)을 매개변수로 전달
    //-함수 내에서 매개변수를 변경하면 원본값도 변경 됨
    //*c언어는 cal-by-value이기 때문에 swap(a,b)의 a와 b가 복사해서 전달되기 때문에 main()함수의 원본 a,b값은 영향을 받지 않음 
    swap(&a,&b);
    printf("a:%d,b:%d",a,b); 
}